...
def filtrar_por_participantes(lista_escolas,participantes):
    return [f for f in lista_escolas if f['participantes'] > 100]
...
def parte2_numpy():
    ...
    mais_de_100 = filtrar_por_participantes(escolas,participantes)
    ...
    abaixo_da_media = medias < medias.mean()
    print(f"Escolas abaixo da média geral: {nomes[abaixo_da_media].tolist()}")

    mais_de_100_participantes = participantes > 100
    print(f"Escolas com mais de 100 participantes: {len(nomes[mais_de_100_participantes].tolist())}")  
